...
(define (help l key)
  (if (null? l) '()
      (begin (displayln "Starting with substitution:")
      (displayln (car l))
      (displayln " ")
      (displayln "Modified Key:")
      (displayln (utils:add-substitution (car l) key))
      (displayln " ")
             (let* ([key1 (utils:add-substitution (car l) key)]
                    [extended-key (algo:dictionary-closure key1)]
                    [complete-key (algo:secret-word-enumeration extended-key)])
               (cond [(equal? extended-key #f) (help (cdr l) key)]
                     [(equal? complete-key #f) (help (cdr l) key)]
                     [else (begin (displayln " ")
                           (utils:show-key complete-key)
                           (displayln " ")
                           (append (list complete-key) (help (cdr l) key)))])))))
...
(define (crack-cipher strategies key)
  (if (null? strategies) (list key)
      (remove-duplicates (appender (map (lambda (x) (crack-cipher (cdr strategies) x)) (help ((car strategies) key) key))))))
